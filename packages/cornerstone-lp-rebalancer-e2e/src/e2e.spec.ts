...
import { bundledVincentPolicy as rebalancerPolicy } from '@reubenr0d/lp-rebalancer-policy';
...
function getSendAbilityClient(ethersSigner: ethers.Wallet) {
  return getVincentAbilityClient({ bundledVincentAbility: rebalancerAbility, ethersSigner });
}
...
const PERMISSION_DATA: PermissionData = {
  [rebalancerAbility.ipfsCid]: {
    [rebalancerPolicy.ipfsCid]: { maxSends: 1, timeWindowSeconds: 20 }, // Only allow 1 transfer every 20 seconds
  },
};
  ...
  it('should send test tokens to the funder account, but be blocked from sending again immediately by the policy', async () => {
    ...
    if (policyFailResult.success === false && policyFailResult.context) {
      expect(policyFailResult.context.policiesContext.deniedPolicy).toHaveProperty(
        'packageName',
        '@reubenr0d/lp-rebalancer-policy',
      );
    }
